...
Generate Initial Concept: Use Perplexity to research your app idea, competitors, and user needs. Log the research and insights into your Notion tracker.
Create Architectural Blueprint: Use Claude (the conversational AI) to draft a project architecture and define features based on the Perplexity research. This blueprint is then added to Notion.
Define Work Plans: Leverage Claude Code to break down the architectural blueprint into actionable, atomic coding tasks. These tasks are added as individual work items to your Notion database.
Human Validation (Mob Elaboration): Review the AI-generated architecture and work plans in Notion. Mark the work items as "Mob Elaboration" when they have been validated.
...
AI-First Coding: Use Cursor as your IDE. Prompt its chat to generate boilerplate code for your FastAPI or Flask app, including the webhook endpoint and database connection logic.
Execute Complex Tasks (Mob Construction): Use Claude Code within Cursor's terminal to perform specific coding tasks. Examples include:
...
Develop the Vercel Function: Create the Python serverless function on Vercel that will:
...
Create Frontend: Use Cursor to generate the mobile-friendly frontend code that fetches and displays the AI-generated results.
GitHub and Secrets: Push your code to GitHub. Add API keys and secrets to both GitHub's and Vercel's secret managers.
Human Validation and Testing: Review and test all AI-generated code. Run tests with the assistance of Claude Code.
...
Configure Webhook: Set up the webhook in your Tally.so form to point to the URL of your Vercel serverless function, including the necessary secret for verification.

Automated Deployment: Push code changes to GitHub to trigger Vercel's automatic build and deployment pipeline.
...
Track in Notion: Use your Notion database to update the status of work items to "Operations," create new items for feedback, and use the timeline view to visualize release cycles.
...
Set up Notion and populate it with initial research and work items.
